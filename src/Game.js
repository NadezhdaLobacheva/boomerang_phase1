...
class Game {
  ...
  regenerateTrack() {
    ...
    this.track[this.hero.boomerang.positionY][this.hero.boomerang.positionX] =
  ...
  async check(fullName, kills) {
    if (
      ...
      this.hero.die();
      console.timeEnd(chalk.yellowBright('Ты жив уже'));
    ...
    if (
      this.hero.boomerang.positionX < this.trackLength - 1 &&
    ...
    if (
      (this.hero.positionX === this.hero.boomerang.positionX &&
        this.hero.positionY === this.hero.boomerang.positionY) ||
      this.hero.boomerang.positionX === 0
    ) {
    ...
    if (
      (this.enemy.positionX === this.hero.boomerang.positionX ||
        this.enemy.positionX === this.hero.boomerang.positionX - 1) &&
      this.enemy.positionY === this.hero.boomerang.positionY
    ) {
      this.kills += 1;
      this.enemy.die();
      this.enemy = new Enemy();
  ...
  play(fullName) {
    console.time(chalk.yellowBright('Ты жив уже'));
    ...
    if (this.intervalId === null) {
      this.intervalId = setInterval(() => {
        if (this.intervalId !== null) {
          ...
          if (this.intervalId !== null) {
            this.regenerateTrack();
            this.view.render(this.track, this.kills, fullName);
          }
        }
      }, 100);
    }
